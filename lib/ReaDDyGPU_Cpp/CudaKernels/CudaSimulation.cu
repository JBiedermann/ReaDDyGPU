...
int CudaSimulation::initialize(){
    ...
    cudaForces=NULL;
    cudaTypes=NULL;
    cudaNeighborList=NULL;
    cudaNeighborListBegins=NULL;
    cudaRDFMatrix=NULL;
    globalRandStates=NULL;
    hostNeighborListBegins=NULL;
    hostNeighborList=NULL;
    ...
    /// ////////////////////////////////////////////////////////////////////////
    if(simulation->cudaDevice<numberOfCudaDevices && simulation->cudaDevice>=0)
    ...
    cuRandSize = simulation->numberParticles;
...
if(simulation->testmode)
    ...
    /// types
    if(copyTypesToDevice()!=0)
        return 1;


    if(initializeNeighborList()!=0){
        cout << "neighbor list initialization failed" << endl;
        return 1;
    }
...
int CudaSimulation::copyRDFMatrix(){
    ...
    if(simulation->testmode){
        ...
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error, RDF natrix copy device to host: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
...
int CudaSimulation::initializeNeighborList(){
    ...
    if(cudaNeighborListBegins!=NULL){
        cudaFree(cudaNeighborListBegins);
    }
    ...
    if(simulation->testmode){
        ...
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "malloc cudaNeighborListBegins, cuda error: %s\n",cudaGetErrorString(error ));
            return 1;
        }
    }

    if(hostNeighborListBegins!=NULL){
        free(hostNeighborListBegins);
    }
...
int CudaSimulation::createNeighborList(){
    ...
    if(cudaNeighborList!=NULL){
        cudaFree(cudaNeighborList);
    }
    ...
    if(simulation->testmode){
        ...
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "malloc cudaNeighborList, cuda error: %s\n",cudaGetErrorString(error ));
            return 1;
        }
    }
    if(hostNeighborList!=NULL){
        free(hostNeighborList);
    }
    ...
    cudaError_t error = cudaGetLastError();
    if ( cudaSuccess != error ){
        printf( "cuda error, create neighborlist: %s\n",cudaGetErrorString(error) );
        return 1;
    }
...
int CudaSimulation::testNeighborList(){
    ...
    cudaError_t error = cudaGetLastError();
    if ( cudaSuccess != error ){
        printf( "cuda error, neighborlist test problem: %s\n",cudaGetErrorString(error) );
        return 1;
    }
...
int CudaSimulation::testNeighborListBigTest(){

    cudaMemcpy(hostNeighborList, cudaNeighborList, ( simulation->numberParticles * 2 * sizeof ( int ) ), cudaMemcpyDeviceToHost);
    cudaMemcpy(hostNeighborListBegins, cudaNeighborListBegins, ( numberOfLatticeFields * sizeof ( int ) ), cudaMemcpyDeviceToHost);
    if(copyPosFromDevice()!=0)
        return 1;
    cudaError_t error = cudaGetLastError();
    if ( cudaSuccess != error ){
        printf( "cuda error, neighborlist test copy problem: %s\n",cudaGetErrorString(error) );
        return 1;
    }
    ...
    int count2 = 0;
    int particleNr;
    for(int currendField=0; currendField<numberOfLatticeFields; ++currendField){
        particleNr=hostNeighborListBegins[currendField];
        //cout << i << ":" << x << endl;
        if(particleNr!=-1){
            do{
                count++;
               // cout << hostNeighborList[2*x+0] << " " << x << " " << hostNeighborList[2*x+1] << endl;
                int field=((int)floor((simulation->coords[3*particleNr+2]-boxSize[4])/maxCutoff)%latticeSize[2])*latticeSize[0]*latticeSize[1]
                         +((int)floor((simulation->coords[3*particleNr+1]-boxSize[2])/maxCutoff)%latticeSize[1])*latticeSize[0]
                         +((int)floor((simulation->coords[3*particleNr+0]-boxSize[0])/maxCutoff)%latticeSize[0]);
                if(field!=currendField){
                    cout << "wrong field!, error in neighborlist!" << endl;
                    return 1;
                }
                particleNr=hostNeighborList[2*particleNr+1];
                if(particleNr==hostNeighborListBegins[currendField])
                    break;
                //char a;
                //cin >> a;
            }while(true);
...
int CudaSimulation::allocateForces(){
    ...
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error, allocate forces: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    return 0;
...
int CudaSimulation::initializeCuRand(int size){


    /*if(globalRandStates!=NULL){
        cudaFree(globalRandStates);
    }*/
    if(simulation->testmode){
        cudaDeviceSynchronize();
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "random variable free, cuda error: %s\n",cudaGetErrorString(error ));
            return 1;
        }
    }
    cudaMalloc ( (void**)&globalRandStates, size * sizeof( curandState ) );
    if(simulation->testmode){
        cudaDeviceSynchronize();
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
    ...
    /// setup seeds
    setup_kernel <<< gridSize, blockSize >>> ( globalRandStates, time(NULL), size );
    if(simulation->testmode){
        cudaDeviceSynchronize();
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
...
int CudaSimulation::initializeCuRand(){
    return initializeCuRand(simulation->numberParticles);
}

int CudaSimulation::shrinkCuRand(){
    int cuRandSize = simulation->numberParticles*1.5;
    curandState * globalRandStatesTemp;

    cudaMalloc ( (void**)&globalRandStatesTemp, cuRandSize * sizeof( curandState ) );
    if(simulation->testmode){
        cudaDeviceSynchronize();
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error, shrinkCuRand1: %s\n",cudaGetErrorString(error ));
            return 1;
        }
    }

    cudaMemcpy(globalRandStatesTemp, globalRandStates, ( cuRandSize * sizeof( curandState )  ), cudaMemcpyDeviceToDevice);
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error, shrinkCuRand2: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    cudaFree(globalRandStates);
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cudaFree error,shrinkCuRand: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    globalRandStates=globalRandStatesTemp;
    return 0;
}

int CudaSimulation::extendCuRand(){
    int oldSize = cuRandSize;
    int cuRandSize = simulation->numberParticles*1.5;
    curandState * globalRandStatesTemp;
    ...
    if(simulation->testmode){
        ...
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error, extendCuRand1: %s\n",cudaGetErrorString(error ));
            return 1;
        }
    }

    cudaMemcpy(globalRandStatesTemp, globalRandStates, ( oldSize * sizeof( curandState )  ), cudaMemcpyDeviceToDevice);
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error, extendCuRand2: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    ...
    /// setup seeds
    setup_kernel <<< gridSize, blockSize >>> ( globalRandStates+oldSize, time(NULL), cuRandSize-oldSize );
    if(simulation->testmode){
        cudaDeviceSynchronize();
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "random variable initialization, cuda error, extendCuRand3: %s\n",cudaGetErrorString(error ));
            return 1;
        }
    }
...
int CudaSimulation::resizeCuRand(){
    if(simulation->numberParticles>cuRandSize){
        if(simulation->testmode)
            cout << "extend cuRand" << endl;
        return extendCuRand();
    }
    else if(simulation->numberParticles*2<cuRandSize){
        if(simulation->testmode)
            cout << "shrink cuRand" << endl;
        return shrinkCuRand();
    }
    return 0;
}

int CudaSimulation::copyNeighborListToDevice(){
    ...
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error,copyNeighborListToDevice : %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    return 0;
...
int CudaSimulation::copyTypesToDevice(){
    ...
    if(cudaTypes!=NULL){
        ...
        if(simulation->testmode){
            cudaError_t error = cudaGetLastError();
            if ( cudaSuccess != error ){
                printf( "cudaFree error, copyTypesToDevice: %s\n",cudaGetErrorString(error) );
                return 1;
            }
        }
        cudaDeviceSynchronize();
    ...
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cudamalloc error, copyTypesToDevice: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    cudaDeviceSynchronize();
    ...
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error, copyTypesToDevice: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    return 0;
...
int CudaSimulation::copyPosToDevice(){
    ...
    if(cudaCoords!=NULL){
        ...
        if(simulation->testmode){
            cudaError_t error = cudaGetLastError();
            if ( cudaSuccess != error ){
                cout << cudaCoords << endl;
                printf( "cudaFree error, copyPosToDevice: %s\n",cudaGetErrorString(error) );
                return 1;
            }
        }
        cudaDeviceSynchronize();
    ...
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cudamalloc error, copyPosToDevice: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    cudaDeviceSynchronize();
    ...
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error, copyPosToDevice: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    return 0;
...
int CudaSimulation::copyPosFromDevice(){
    ...
    if(simulation->testmode){
        cudaError_t error = cudaGetLastError();
        if ( cudaSuccess != error ){
            printf( "cuda error, copyPosFromDevice: %s\n",cudaGetErrorString(error) );
            return 1;
        }
    }
    return 0;
        ...
        int * cudaSemaphore)
{
    ...
    /// assign one voxel to one cuda block
    int field = blockIdx.x;
    /// check whether voxel is empty
    int particleNumber = cudaNeighborListBegins[field];
    if(particleNumber==-1){
        return;
    }

    /// use shared memory to reuse loded coordinates. Store coordinates, forces, etc. in shared memory for faster memory access
    ...
    /// use float3 for better memory throughput
    float3* myCudaCoords = reinterpret_cast<float3*>(cudaCoords);
    //float3* myCudaForces = reinterpret_cast<float3*>(cudaForces);
    float3* myLocalCoords = reinterpret_cast<float3*>(localParticleCoords);
    float3* myLocalForces = reinterpret_cast<float3*>(localParticleForces);
    float3* myLocalInteractingParticleCoords = reinterpret_cast<float3*>(localInteractingParticleCoords);
    float3* myLocalInteractingParticleForces = reinterpret_cast<float3*>(localInteractingParticleForces);
    ...
    /// get IDs from surrounding voxels
    ...
    do{
        ...
        /// store particle properties in shared mem
        if(loadedAParticle){
            //uint4* p = reinterpret_cast<uint4*>(in);
            myLocalCoords[thread]=myCudaCoords[particleNumber];
            myLocalForces[thread]=make_float3(0.0,0.0,0.0);
            /*for(int dim=0; dim<3; ++dim){
        ...
        /// for all interacting particles in this field ///////////////////////////////////////////////////////////////////
        /// calculate interactions between all particles in this voxel
        ...
        /// iterate over and load particles
        do{
            ...
            /// store interacting particle properties in shared mem
            if(loadedAInteractingParticle){
                myLocalInteractingParticleCoords[thread]=myCudaCoords[interactingParticleNumber];
                /*for(int dim=0; dim<3; ++dim){
                    localInteractingParticleCoords[3*thread+dim]=cudaCoords[3*interactingParticleNumber+dim];
                    /// no need to store the forces of the interacting particles again
        ...
        /// for all interacting particles in neighboring fields ///////////////////////////////////////////////////////////////////
        ...
        do{
            ...
            if(loadedAInteractingParticle){
                myLocalInteractingParticleCoords[thread]=myCudaCoords[interactingParticleNumber];
                myLocalInteractingParticleForces[thread]=make_float3(0.0,0.0,0.0);
                /*for(int dim=0; dim<3; ++dim){
                    localInteractingParticleCoords[3*thread+dim]=cudaCoords[3*interactingParticleNumber+dim];
                    localInteractingParticleForces[3*thread+dim]=0;
                }*/
