...
__device__ void warpCalculateOrderTwoPotential(float * particleCoord, float * particleForce, int particleType, float * interactingParticleCoord, float * interactingParticleForce, int interactingParticleType, int orderTwoPotentialNr, CudaOrderTwoPotential * cudaCudaOrderTwoPotentials, float * cudaBoxSize, int numberOfParticleTypes, float * cudaCollisionRadiiMatrix, bool atomicAble);
...
__device__ void calculateOrderTwoPotential(int particleNumber, int interactingParticle, int orderTwoPotentialNr, float r, float* cudaCoords, float* cudaForces, int* cudaTypes, CudaOrderTwoPotential * cudaCudaOrderTwoPotentials, int numberOfParticleTypes, float * cudaCollisionRadiiMatrix, bool atomicAble){
    ...
    if(cudaCudaOrderTwoPotentials[orderTwoPotentialNr].type==1){        /// Harmonic Potential
        ...
        for(int coord=0; coord<3; ++coord){
            ...
            if(cudaCudaOrderTwoPotentials[orderTwoPotentialNr].subtype==1 && r>r0){ /// attractive
                atomicCasAdd(&cudaForces[particleNumber*3+coord], -force , atomicAble);
                atomicCasAdd(&cudaForces[interactingParticle*3+coord], force , atomicAble);
            }
                    ...
                    do
                    {
                        ...
                        if(interactingParticle<particleNumber){
                            ...
                            for(int orderTwoPotentialNr=0; orderTwoPotentialNr<numberOfOrderTwoPotentials; ++orderTwoPotentialNr){
                                if(cudaOrderTwoPotentialsMatrix[cudaTypes[particleNumber]*numberOfParticleTypes*numberOfOrderTwoPotentials+cudaTypes[interactingParticle]*numberOfOrderTwoPotentials+orderTwoPotentialNr]!=0 && r<=maxCutoff){
                                //if(cudaOrderTwoPotentialsMatrix[cudaTypes[particleNumber]*numberOfParticleTypes+cudaTypes[interactingParticle]*numberOfParticleTypes+orderTwoPotentialNr]!=0 && r<=maxCutoff){
                                    calculateOrderTwoPotential(particleNumber, interactingParticle, orderTwoPotentialNr, r, cudaCoords, cudaForces, cudaTypes, cudaCudaOrderTwoPotentials, numberOfParticleTypes, cudaCollisionRadiiMatrix, atomicAble);
                                    //__device__ void warpCalculateOrderTwoPotential(float * particleCoord, float * particleForce, int particleType, float * interactingParticleCoord, float * interactingParticleForce, int interactingParticleType, int orderTwoPotentialNr, CudaOrderTwoPotential * cudaCudaOrderTwoPotentials, float * cudaBoxSize, int numberOfParticleTypes, float * cudaCollisionRadiiMatrix, bool atomicAble){
                                    //warpCalculateOrderTwoPotential(&cudaCoords[particleNumber*3], &cudaForces[particleNumber*3], cudaTypes[particleNumber], &cudaCoords[interactingParticle*3], &cudaForces[interactingParticle*3], cudaTypes[interactingParticle], orderTwoPotentialNr, cudaCudaOrderTwoPotentials, cudaBoxSize, numberOfParticleTypes, cudaCollisionRadiiMatrix, 1);
                                }
                            }
                        }
        ...
        bool atomicAble)
{
    ...
    //if(thread==0){
    //    printf("%p %p %i %i %i \n", s, &numberOfLoadedInteractingParticles[1], (unsigned int)numberOfLoadedInteractingParticles - (unsigned int)s, (int)(warpsize*3*4*sizeof(float)+(27+warpsize*3+7)*sizeof(int)), (warpsize*3*4+(27+warpsize*3+7)));
    //}
...
//cudaTest(s, localParticleCoords);
        ...
        do{
            ...
            for(int i=0; i<=thread; i++){
                ...
                /// empty field
                    ...
                    ++x;
                    if(x<=13){
//if((long long int)&cudaNeighborListBegins[todo[x]] > (long long int)0x1400000000)
//    printf("1 %p %i , %i; %p %p \n", &cudaNeighborListBegins[todo[x]], todo[x], x, cudaNeighborListBegins, todo);
...
//if((long long int)&cudaNeighborList[interactingParticleNumber*2+1] > (long long int)0x1400000000)
//    printf("2 %p \n", &cudaNeighborList[interactingParticleNumber*2+1]);
                    /// take next field
                    ++x;
                    if(x<=13){
//if((long long int)&cudaNeighborListBegins[todo[x]] > (long long int)0x1400000000)
...
//if((long long int)&cudaCoords[3*interactingParticleNumber+dim] > (long long int)0x1400000000)
//    printf("4.1 %p \n", &cudaCoords[3*interactingParticleNumber+dim]);
            ...
            while(calculationCycle*warpsize+thread<numberOfLoadedParticles[0]*numberOfLoadedInteractingParticles[0]){
                ...
                //if(localParticleTypes[particleToCalculate]>2 || localInteractingParticleTypes[interactingParticleToCalculate]>2){
                //    printf("%i(%i, %i), %i(%i, %i)\n", localParticleTypes[particleToCalculate], particleToCalculate, numberOfLoadedParticles[0], localInteractingParticleTypes[interactingParticleToCalculate],  interactingParticleToCalculate, numberOfLoadedInteractingParticles[0]);
                //}
                for(int orderTwoPotentialNr=0; orderTwoPotentialNr<numberOfOrderTwoPotentials; ++orderTwoPotentialNr){
...
//printf("\t\t\t\t\t %f  %f  %f  \n", localInteractingParticleForces[thread*3],localInteractingParticleForces[thread*3+1],localInteractingParticleForces[thread*3+2]);
                for(int dim=0; dim<3; ++dim){
...
//printf("%f  %f  %f  \n", localParticleForces[thread*3],localParticleForces[thread*3+1],localParticleForces[thread*3+2]);
            for(int dim=0; dim<3; ++dim){
                atomicCasAdd(&cudaForces[particleNumber*3+dim], localParticleForces[thread*3+dim]/2, atomicAble); // devide by two, because the forces are computed twice (in this field), due to force symmetry(for the other fields)
//if((long long int)&cudaForces[particleNumber*3+dim] > (long long int)0x1400000000)
//    printf("5 %p \n", &cudaForces[particleNumber*3+dim]);
            }
        }
        ...
        if(thread==warpsize-1){
            if(cudaNeighborList[particleNumber*2+1]!=cudaNeighborListBegins[field]){
//if((long long int)&cudaNeighborList[particleNumber*2+1] > (long long int)0x1400000000)
//    printf("6 %p \n", &cudaNeighborList[particleNumber*2+1]);
//if((long long int)&cudaNeighborListBegins[field] > (long long int)0x1400000000)
//    printf("7 %p \n", &cudaNeighborListBegins[field]);
...
__global__ void groups(float* cudaCoords, float* cudaForces, int* cudaTypes, int numberParticles, int numberOfParticleTypes, float * cudaCollisionRadiiMatrix, CudaOrderTwoPotential * cudaCudaGroupPotentials, GroupPart * cudaIndividualGroups, int numberOfIndividualGroups, float * cudaBoxSize, bool atomicAble){
    ...
    if(pairNumber<numberOfIndividualGroups){
        ...
        calculateOrderTwoPotential(cudaIndividualGroups[pairNumber].particle1, cudaIndividualGroups[pairNumber].particle2, cudaIndividualGroups[pairNumber].groupPot, r, cudaCoords, cudaForces, cudaTypes, cudaCudaGroupPotentials, numberOfParticleTypes, cudaCollisionRadiiMatrix, atomicAble);
    }
}
...
//void warpOrderTwo(int block, int warpsize, float* cudaCoords, float* cudaForces, int* cudaTypes, int * cudaNeighborListBegins, int * cudaNeighborList, int * cudaLatticeSize, float * cudaBoxSize, curandState* globalRandStates, int numberParticles, int maxCutoff, int * cudaOrderTwoPotentialsMatrix, CudaOrderTwoPotential * cudaCudaOrderTwoPotentials, int numberOfOrderTwoPotentials, int numberOfParticleTypes, float * cudaCollisionRadiiMatrix){
            ...
            do{
                ...
                for(int thread=0; thread<32; ++thread){
                                    ...
                                    for(int i=0; i<=thread; i++){
                                                ...
                                                /// if we reach the begin of the field (first particle in fields list) again (not valid for very first particle)
                                                /*else if(interactingParticleNumber[thread] == cudaNeighborListBegins[todo[x[thread]]] && !load_first_particle[thread]){
                                                    /// take next field
                                                    ++x[thread];
                                                    if(x[thread]<=13){
                                                        interactingParticleNumber[thread] = cudaNeighborListBegins[todo[x[thread]]];
...
/*
    int count =0;
    for(int block=0; block<numberOfLatticeFields; ++block){
        //void warpOrderTwo(int block, int warpsize, float* cudaCoords, float* cudaForces, int* cudaTypes, int * cudaNeighborListBegins, int * cudaNeighborList, int * cudaLatticeSize, float * cudaBoxSize, curandState* globalRandStates, int numberParticles, int maxCutoff)
        testWarpOrderTwo(&count, block, warpsize, simulation->coords, new float[simulation->numberParticles*3], simulation->types, hostNeighborListBegins, hostNeighborList, latticeSize, boxSize, globalRandStates, simulation->numberParticles, maxCutoff);
    }
    cout  << "interactions: "<<count<< " of " << simulation->numberParticles << endl;
*/
